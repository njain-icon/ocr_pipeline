...
from ocr_pipeline.app.config.models import (
    Config,
    DaemonConfig,
    LoggingConfig,
...
@contextmanager
def config_context(config_dict: dict):
    """
    Context manager to set configuration for a thread.
    
    Args:
        config_dict: The configuration dictionary to set for the current context
    """
    token = var_server_config_dict.set(config_dict)
    try:
        yield
    finally:
        var_server_config_dict.reset(token)
